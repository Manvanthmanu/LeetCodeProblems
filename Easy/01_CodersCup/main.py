...
# an n * n  matrix is valid if everyrow and every column contains all the integers from 1 to n( inclusive )

# Given an n* n integer matrix, matrix , return true , if the matrix is valid otherwisse  , return false 

# example  input : matrix = [[1,2,3],[3,1,2],[2,3,1]] , output : true , Explanation : In the Case , n = 3 and every row and column contains the numbers 1,2and 3 hence true 
...
def Solution(matrix:list[list[int]]) -> bool:
    ...
    # # for rows 
        for i in matrix:
            # check if its a square matrix ( condition n*n )
            sets = set(i)
            lists = list(sets)
            lists.sort()
            if len(sets) != n:
    ...
    # for columns
        ...
        for i in range(1,n+1):
            newlist = []
            for j in matrix:
                newlist.append(j[i-1])
            embededlist.append(newlist)
        
        for i in embededlist:
            # check if its a square matrix ( condition n*n )
